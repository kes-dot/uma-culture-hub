...
const CareerPathways = () => {
  const { careerPathways } = brandConfig;

  return (
    <section id="careers" className="py-24 gradient-warm">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="text-sm font-semibold text-primary uppercase tracking-wider">{careerPathways.badge}</span>
          <h2 className="text-3xl sm:text-4xl font-bold text-foreground mt-3 mb-4">
            {careerPathways.heading}
          </h2>
          <p className="text-muted-foreground text-lg">
            {careerPathways.subheading}
          </p>
        </div>

        <div className="max-w-4xl mx-auto">
          {careerPathways.steps.map((step, i) => (
            <div key={step.title} className="flex gap-6 mb-8 last:mb-0 animate-fade-in" style={{ animationDelay: `${i * 0.15}s` }}>
              ...
              <div className="flex flex-col items-center">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${
                  i === careerPathways.steps.length - 1
